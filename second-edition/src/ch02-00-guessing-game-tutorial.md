...
Первая часть программы будет запрашивать у пользователя ввод данных,
обрабатывать эти данные и проверять что ввод пользователя вообще корректен.
Для начала дадим пользователю возможность ввести предположение. Поместите код
из Листинга 2-1 в файл *src/main.rs*.
...
<span class="caption">Листинг 2-1: Код для получения и вывода на экран предположения пользователя</span>

За этим кодом скрывается огромное количество информации, так что давайте
разберем его шаг за шагом. Чтобы получать вводимую пользователем информацию
и затем выводить результат нам понадобится библиотека ввода-вывода `io`
(input/output), мы должны явным образом добавить её в область видимости.
Библиотека `io` является частью стандартной библиотеки (которая так же
известна как `std`):
...
По умолчанию Rust импортирует в программу лишь небольшой набор самых необходимых
вещей — они называются [*прелюдией*][prelude]<!-- ignore --> (prelude). Если тип, который
вы хотите использовать, не содержится в прелюдии, вы должны явным образом добавить
этот тип в область видимости с помощью оператора `use`. Библиотека `std::io`
предоставляет большое количество полезных инструментов, связанных с вводом/выводом,
включая возможность получения вводимой пользователем информации.

[prelude]: ../../std/prelude/index.html

Как вы помните из первой главы, программа начинает исполняться с функции `main`:

```rust,ignore
fn main() {
```

С помощью `fn` мы указываем, что хотим объявить функцию, круглые скобочки `()` говорят
о том, что эта функция не имеет аргументов и фигурная скобка `{` показывает, что
здесь начинается тело функции.

В первой главе также упоминалось, что `println!` это макрос, который позволяет вывести
данные на экран:
...
Этот код просто выводит информацию о том, что это за игра и просит пользователя ввести
его предположение.
